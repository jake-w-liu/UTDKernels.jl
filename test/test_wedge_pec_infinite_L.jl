...
@testset "far-field local pole evaluation preserves nonzero detunings" begin
    ...
    function gtd_cot_ref(phi)
        ...
        prefactor = UTDKernels.pec_wedge_prefactor(k, n)
        ds_sum = zero(ComplexF64); dh_sum = zero(ComplexF64)
        for j in 1:4
            cj = cot(t.psi[j])
            ds_sum += UTDKernels.PEC_SIGMA_SOFT[j] * cj
            dh_sum += UTDKernels.PEC_SIGMA_HARD[j] * cj
        end
        return (prefactor * ds_sum, prefactor * dh_sum)
    end
    ...
    # The sweep extends through the former dead window and four tighter decades.
    for delta in (1.0e-10, 1.0e-9, 1.0e-8, 1.0e-7,
                  1.0e-6, 1.0e-5, 5.0e-5, 1.0e-4, 1.5e-4, 1.7e-4)
        phi = π + phip - delta
    ...
    # The same tight nonzero detuning must not be clamped to the finite-L
    # exact-transition surrogate when kL is large.
    delta = 1.0e-9
    phi = π + phip - delta
    L = 1.0e20
    terms = UTDKernels.kp_four_terms(phi, phip, n)
    values = ntuple(4) do j
        cot(terms.psi[j]) * F_utd(k * L * terms.aj[j])
    end
    C = UTDKernels.pec_wedge_prefactor(k, n)
    Ds_ref = C * sum(UTDKernels.PEC_SIGMA_SOFT[j] * values[j] for j in 1:4)
    Dh_ref = C * sum(values)
    Ds, Dh = pec_wedge_DsDh(wedge, RayAngles(phi, phip), k, L)
    @test Ds ≈ Ds_ref rtol=1e-8
    @test Dh ≈ Dh_ref rtol=1e-8
end
