...
function gen_fig1()
    ...
    CSV.write(joinpath(DATA_DIR, "fig1_transition_function.csv"), df)
    println("  fig1: transition function  ($(nrow(df)) rows)")
end
...
function gen_fig2()
    ...
    for (krho, label) in [(5.0, "krho5"), (50.0, "krho50")]
        ...
        df = DataFrame(rows)
        CSV.write(joinpath(DATA_DIR, "fig2_halfplane_$(label).csv"), df)
        println("  fig2 ($label): half-plane UTD vs exact  ($(nrow(df)) rows)")
    end
end
...
function gen_fig3()
    ...
    df = DataFrame(rows)
    CSV.write(joinpath(DATA_DIR, "fig3_shadow_boundary.csv"), df)
    println("  fig3: shadow boundary detail  ($(nrow(df)) rows)")
end
...
function gen_fig4()
    ...
    df = DataFrame(rows)
    CSV.write(joinpath(DATA_DIR, "fig4_gtd_convergence.csv"), df)
    println("  fig4: GTD convergence  ($(nrow(df)) rows)")
end
...
function gen_fig5()
    ...
    df = DataFrame(rows)
    CSV.write(joinpath(DATA_DIR, "fig5_wedge_patterns.csv"), df)
    println("  fig5: multi-wedge patterns  ($(nrow(df)) rows)")
end
...
function gen_fig6()
    ...
    CSV.write(joinpath(DATA_DIR, "fig6a_ad_vs_fd_phi.csv"), DataFrame(rows_phi))
    println("  fig6a: AD vs FD wrt φ  ($(length(rows_phi)) rows)")
    ...
    CSV.write(joinpath(DATA_DIR, "fig6b_ad_vs_fd_k.csv"), DataFrame(rows_k))
    println("  fig6b: AD vs FD wrt k  ($(length(rows_k)) rows)")
    ...
    CSV.write(joinpath(DATA_DIR, "fig6c_ad_vs_fd_L.csv"), DataFrame(rows_L))
    println("  fig6c: AD vs FD wrt L  ($(length(rows_L)) rows)")
end
...
function gen_fig7()
    ...
    df = DataFrame(rows)
    CSV.write(joinpath(DATA_DIR, "fig7_branch_safety.csv"), df)
    n_inf = count(.!df.Ds_naive_finite)
    println("  fig7: branch safety  ($(nrow(df)) rows, $n_inf naïve NaN/Inf)")
end
