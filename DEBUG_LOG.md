...
## 2026-08-20 — The root manifest contradicted the declared Julia minimum

Symptom: the package declares `julia = "1.9"`, but a Julia 1.9.4
`Pkg.instantiate()` could not resolve the tracked root manifest because it
pinned SpecialFunctions 2.6.1, which requires Julia 1.10. `Pkg.test()` appeared
to work only because Pkg discarded those exact versions in its temporary test
environment and resolved SpecialFunctions 2.5.1 instead.

Root cause: the only root manifest had been generated by Julia 1.12.6. Its
standard-library entries and SpecialFunctions pin were not compatible with the
declared minimum. Replacing it with a Julia 1.9 manifest alone made Julia 1.12
warn that the environment had been resolved by another Julia version.

Fix: regenerate `Manifest.toml` with Julia 1.9.4 as the backward-compatible
fallback, and add `Manifest-v1.12.toml` resolved by Julia 1.12.7. Julia 1.12
selects the version-specific manifest, while Julia 1.9 ignores that newer
feature and selects the fallback manifest.

Verification:

- Julia 1.9.4 selected `Manifest.toml`, instantiated without re-resolution,
  and passed all 11,662 tests with SpecialFunctions 2.5.1.
- Julia 1.12.7 selected `Manifest-v1.12.toml`, instantiated without manifest
  warnings, and passed all 11,662 tests with SpecialFunctions 2.9.0.

Lesson: a library that supports Julia versions with incompatible dependency
